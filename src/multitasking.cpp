	...
	m_multitaskingModel(new MultitaskingModel)
{
    ...
    connect(effects, &EffectsHandler::numberDesktopsChanged, this, &MultitaskingEffect::onNumberScreensChanged);
...
{
    qCDebug(BLUR_CAT) << "------- screens changed " << effects->screens();
}
...
QVariantList MultitaskingEffect::windowsFor(const EffectScreen *screen, int desktop)
{
    QVariantList vl;
    QDesktopWidget dw;
    for (const auto& ew: effects->stackingOrder()) {
        if (isRelevantWithPresentWindows(ew) && ew->isOnAllDesktops() && effects->screenAt(ew->pos()) == screen) {
            auto wid = findWId(ew);
            assert (effects->findWindow(wid) == ew);
            vl.append(wid);
        }
        if (isRelevantWithPresentWindows(ew) && ew->desktop() == desktop) {
            if (effects->screenAt(ew->pos()) == screen) {
                auto wid = findWId(ew);
                assert (effects->findWindow(wid) == ew);
                vl.append(wid);
            }
...
void MultitaskingEffect::moveWindow2Desktop(EffectScreen *screen, int desktop, QVariant winId)
{
...
void MultitaskingEffect::setActive(bool active)
{
    ...
    if (m_multitaskingViewVisible) {
        ...
        if (!m_multitaskingView) {
            ...
            m_multitaskingView->rootContext()->setContextProperty("numScreens", effects->screens().length());
...
void MultitaskingEffect::calculateWindowTransformationsClosest(EffectWindowList windowlist, const EffectScreen *screen,
...
void MultitaskingEffect::removeEffectWindow(const EffectScreen *screen, int desktop, QVariant winid)
{
